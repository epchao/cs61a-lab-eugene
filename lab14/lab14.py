...
def add_trees(t1, t2):
    ...
    if len(t1.branches) < len(t2.branches):
        t1.branches += [0 for _ in range(len(t1.branches), len(t2.branches))]
    if len(t1.branches) > len(t2.branches):
        t2.branches += [0 for _ in range(len(t2.branches), len(t1.branches))]
    return Tree(t1.label + t2.label, [add_trees(b1,b2) for b1, b2 in zip(t1.branches, t2.branches)])
